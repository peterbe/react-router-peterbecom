...
import { type Comment, blogitems } from "./blogitems.ts"
      ...
      for (const blogitem of blogitems) {
        for (const category of blogitem.categories) {
          for (const oc of ocs) {
            if (
              oc !== category &&
              oc.toLowerCase() === category.toLowerCase()
            ) {
...
router.get("/v1/plog/", (req: Request, res: Response, next: NextFunction) => {
